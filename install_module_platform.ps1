<#
.SYNOPSIS
    Name: Install_module_platform.ps1
...
function web {
    ...
    If((Test-Path $global:module_excel)-eq $true){
        ...
        Write-LogLine "$module module not found"
        Write-LogLine "Installing $module from the internet!"
        Install-Module -Name $module -AllowClobber -PassThru -Force
        Start-Sleep 3
        Write-LogLine "$module installed!"
    }
}
#--------------------(Main)-----------------------------------------------------------------------------
Set-logInit("Install Pwsh module true platorm  _")
Write-LogLine("$date Install Pwsh module True platform")
Write-Version  $version
Write-Version $space

if($input -match "local"){
    Write-LogLine "Running Local function"
     Write-Host "Running Local function"
     local
}elseif ($input -match "web"){
    Write-LogLine "Running Web function"
    Write-Host "Running Web function"
    Web
} else{
    Write-host $global:error
    Write-LogLine $global:error
}
